...
df1 <- read.csv("H:/Backup_I_J_drive/ScottNess/Anitha/2024/merged.gene_counts.tsv", header = TRUE, sep = "\t") #salmon.merged.gene_counts
colnames(df1)[1] <- "gene_name"
df1 <- aggregate(. ~ gene_name, data = df1, mean) # aggregate(. ~ gene_name, data = df1[,-1], mean)
gene_counts <- df1[,-1]
...
# KEGG enrichment
ekg <- enrichKEGG(gene = deg_sig$ENTREZID, organism = "hsa", pvalueCutoff = 0.1, qvalueCutoff = 0.2) # mmu
...
reactome_enrich <- enrichPathway(gene = deg_sig$ENTREZID, organism = "human",
                                 pAdjustMethod = "BH", pvalueCutoff = 0.1, qvalueCutoff = 0.2, readable = TRUE) # mouse
...
msig_mouse_hallmark <- msigdbr(species = "Homo sapiens", collection = "H") %>%
   dplyr::select(gs_name, gene_symbol) # Mus musculus
